...
4. **Campaign:** arms G (grouped countries), H (us / europe / rest), I (20% cap) × 3 seeds on
   the round_4 data and the arm-A recipe, with round_4 arm A as the existing control; J is the
   ablation. 12 configs are generated and self-tested; ~$30 / ~10 pod-hours for the nine core
   runs. Pre-registered decision rule in section 4.
...
## 3. The proposal: three experiments (plus one ablation)
...
50 epochs, `val_f1` checkpointing), seeds 42/43/44, `runpod.github_branch: develop`. The only
delta is `training.region_balancing`. The control is the existing round_4 arm A
(`world_v10_fourclass_r4_a_s{42,43,44}`, already collected).
...
| **J** ablation | as G but `class_conditional: false` | Uniform group shares with each group's natural class mix; class prior drifts to ~58% NotFarm. | Isolates the anti-shortcut term: if J ≈ G on the diagnostics, the share law is doing the work; if G ≫ J, class conditioning is. Ordered last in the fleet so it is the run that gets stranded if the budget reserve trips. |
...
**Design.** 3 single-lever arms (G, H, I) + 1 ablation (J) × 3 seeds, against round_4 arm A
(3 seeds). Same estimands and statistics as `experiments/EVAL_METHODS.md`: `dAUC_rec` (difference
of mean per-seed AUC) is primary, `SE_total = sqrt(SE_boot² + σ_seed²(1/n₁ + 1/n₂))` with σ_seed
pooled per slice, Holm across the confirmatory family, practical floor 0.005 AUC.
...
J decomposes G only. (ii) ESS differs by arm (G ≈ 0.3, I ≈ 0.5, H ≈ 0.7), so "same epochs" is
not "same number of distinct rows seen". (iii) The `val` diagnostics are on the checkpoint
selection slice. (iv) σ_seed is measured with 3 seeds per arm; the decision band is wide.

**Cost.** 9 core runs (+3 for J) at ~3 h on an RTX 4090 (~$0.34–0.74/h) plus one full-world
scoring pass each ≈ **$30–40 total**, ~10 wall-clock hours at 4 concurrent pods.
...
# 2. launch: G/H/I first, J last; separate state file so the round_4 state is untouched
python3 experiments/launch_fleet.py --order-file experiments/balancing_order.txt \
    --state experiments/results/balancing_fleet_state.json --max-concurrent 4 --budget 15

# 3. collect (pulls sampling_report.json alongside the metrics + scored parquet)
python3 experiments/collect_results.py --watch --names-file experiments/balancing_order.txt

# 4. full-world scoring configs for the finished runs (needed for complete evaluation slices)
python3 experiments/gen_score_configs.py            # picks up world_v10_fourclass_r4_{g,h,i,j}_*
python3 experiments/launch_fleet.py --order-file <score names> --steps inference ...

# 5. evaluate against round_4 arm A
python3 experiments/evaluate_balancing.py           # -> experiments/results/balancing_evaluation.json
```

Before launching, confirm the pod branch: every generated config sets
`runpod.github_branch: develop`, and the sampler must be on that branch (`training/balancing.py`
present). Verify locally with `python tests/test_balancing.py` and
`python tests/test_evaluate_balancing_smoke.py`.

---

## 6. Follow-ups not in this campaign
